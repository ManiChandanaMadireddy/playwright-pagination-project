...
> 47 |     const text = await this.pageInfo.textContent();
  ...
  63 |   async getAllRowsAcrossPages() {
  64 |     let totalRows = 0;
  65 |     const totalPages = await this.getTotalPages();
  66 | 
  67 |     console.log(`Total Pages: ${totalPages}`);
  68 | 
  69 |     for (let i = 1; i <= totalPages; i++) {
  70 |       const rows = await this.getRowCount();
  71 |       console.log(`Page ${i} Rows: ${rows}`);
  72 | 
  73 |       totalRows += rows;
  74 | 
  75 |       // go to next page
  76 |       if (i < totalPages) {
  77 |         await this.goToNextPage();
  78 |       }
  79 |     }
  80 | 
  81 |     return totalRows;
  ...
  83 | }
  84 | 
  85 | module.exports = { PaginationPage };
  86 |  
```