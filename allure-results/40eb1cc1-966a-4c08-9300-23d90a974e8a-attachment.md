...
```ts
  ...
  54 |     await webTable.searchUser(testData.user1.firstName);
  55 |     await webTable.deleteUser();
  56 |     // Validation (no specific user)
> 57 |     await expect(webTable.tableRows.first()).not.toContainText("John");
     |                                                  ^ Error: expect(locator).not.toContainText(expected) failed
  58 |   });
  59 | 
  60 | });
