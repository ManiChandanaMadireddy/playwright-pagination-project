...
Call log:
  - waiting for locator('.rt-tbody .rt-tr-group').first() to be visible
...
> 27 |       await this.page.locator('.rt-tbody .rt-tr-group').first().waitFor({ state: 'visible' });
     |                                                                 ^ TimeoutError: locator.waitFor: Timeout 10000ms exceeded.
  28 |     }
  29 |   
  30 |     // ✅ Get number of visible rows
  31 |     async getRowCount() {
  32 |       return await this.tableRows.count();
  33 |     }
  34 |   
  35 |     // ✅ Search/filter user
  36 |     async searchUser(name) {
  37 |       await this.searchBox.fill(name);
  38 |       await this.page.waitForTimeout(1000); // allow filtering
  39 |     }
  40 |   
  41 |     // ✅ Select rows per page (10,20,30...)
  42 |     async selectPageSize(size) {
  43 |       await this.pageSizeDropdown.selectOption(size.toString());
  44 |     }
  45 |  
  46 | // ✅ Get total pages from "Page X of Y"
  47 | async getTotalPages() {
  48 |     const text = await this.pageInfo.textContent();
  49 | 
  50 |     console.log("Page Info:", text);
  51 | 
  52 |     const match = text.match(/of (\d+)/);
  53 |     return match ? parseInt(match[1]) : 1;
  54 |   }
  55 | 
  56 |   // ✅ Navigate to next page
  57 |   async goToNextPage() {
  58 |     if (await this.nextButton.isEnabled()) {
  59 |       await this.nextButton.click();
  60 |     }
  61 |   }
  62 | 
  63 |   // ✅ Get all rows across pages
  64 |   async getAllRowsAcrossPages() {
  65 |     let totalRows = 0;
  66 |     const totalPages = await this.getTotalPages();
  67 | 
  68 |     console.log(`Total Pages: ${totalPages}`);
  69 | 
  70 |     for (let i = 1; i <= totalPages; i++) {
  71 |       const rows = await this.getRowCount();
  72 |       console.log(`Page ${i} Rows: ${rows}`);
  73 | 
  74 |       totalRows += rows;
  75 | 
  76 |       // go to next page
  77 |       if (i < totalPages) {
  78 |         await this.goToNextPage();
  79 |       }
  80 |     }
  81 | 
  82 |     return totalRows;
  ...
  84 | }
  85 | 
  86 | module.exports = { PaginationPage };
  87 |  
```