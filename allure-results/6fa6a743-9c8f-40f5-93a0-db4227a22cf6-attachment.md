...
- Location: tests\webTables.spec.js:52:3
...
```ts
  ...
  50 | 
  51 |   // ✅ Delete Record
  52 |   test('should delete a user', async () => {
  53 |     await webTable.deleteUser();
  54 | 
  55 |     // Validation (no specific user)
> 56 |     await expect(webTable.tableRows.first()).not.toContainText("John");
     |                                                  ^ Error: expect(locator).not.toContainText(expected) failed
  57 |   });
  58 | 
  59 | });
