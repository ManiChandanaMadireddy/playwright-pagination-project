...
- Name: pagination.spec.js >> 📄 Pagination Validation Suite >> ✅ Validate rows per page selection (10)
- Location: tests\pagination.spec.js:33:1
...
```ts
  ...
  128 | 
  129 |   
  130 | // ✅ Validate rows as per selected dropdown value
  131 | async validateRowsPerPage(expectedSize) {
  132 | 
  133 |   // 🔽 Select page size (10, 20, etc.)
> 134 |   await this.pageSizeDropdown.selectOption(expectedSize.toString());
      |                               ^ TimeoutError: locator.selectOption: Timeout 10000ms exceeded.
  135 | 
  136 |   // ⏳ wait for table refresh
  137 |   await this.page.waitForTimeout(1000);
  138 | 
  139 |   // ✅ Get total pages
  140 |   const totalPages = await this.getTotalPages();
  141 | 
  142 |   console.log(`Total Pages: ${totalPages}`);
  143 | 
  144 |   // ✅ Loop through each page
  145 |   for (let pageIndex = 1; pageIndex <= totalPages; pageIndex++) {
  146 | 
  147 |     // ✅ Get visible rows using your logic
  148 |     const rowCount = await this.getRowCount();
  149 | 
  150 |     console.log(`Page ${pageIndex} → Rows: ${rowCount}`);
  151 | 
  152 |     if (totalPages === 1) {
  153 |       // ✅ Single page → rows should be <= dropdown value
  154 |       if (rowCount <= expectedSize) {
  155 |         console.log(`✅ Single page valid → ${rowCount} rows`);
  156 |       } else {
  157 |         throw new Error(`❌ More rows than expected: ${rowCount}`);
  158 |       }
  159 |     } else {
  160 |       // ✅ Multiple pages → each page should have exact count (except last)
  161 |       if (pageIndex < totalPages) {
  162 |         if (rowCount !== expectedSize) {
  163 |           throw new Error(`❌ Page ${pageIndex} has wrong rows: ${rowCount}`);
  164 |         }
  165 |       } else {
  166 |         // ✅ Last page can have <= expectedSize
  167 |         if (rowCount > expectedSize) {
  168 |           throw new Error(`❌ Last page invalid row count: ${rowCount}`);
  169 |         }
  170 |       }
  171 |     }
  172 | 
  173 |     // 👉 Go to next page
  174 |     if (pageIndex < totalPages) {
  175 |       await this.goToNextPage();
  176 |     }
  177 |   }
  178 | }
  179 | 
  ...
  181 | }
  182 | 
  183 | module.exports = { PaginationPage };
  184 | 
  185 | 
```